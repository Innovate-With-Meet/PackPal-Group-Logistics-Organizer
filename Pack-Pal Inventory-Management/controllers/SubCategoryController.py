...
async def deleteSubCategoryBySubCategoryId(subcategory_id:str):
    result = await sub_category_collection.find_one({"_id" : ObjectId(subcategory_id)})
    
    if result:
        res = await sub_category_collection.delete_one({"_id" : ObjectId(subcategory_id)})
        if res:
            return JSONResponse(status_code=200,detail="Sub-category deleted successfully.")
        else:
            raise HTTPException(status_code=500,detail="Server Error")
    else:
        raise HTTPException(status_code=404,detail="Sub-Category not found")

async def deleteSubCategoryByCategoryId(category_id:str):
    results = await sub_category_collection.find({"category_id" : ObjectId(category_id)}).to_list()
    for result in results:
        res = await sub_category_collection.delete_one({"category_id" : ObjectId(category_id)})
