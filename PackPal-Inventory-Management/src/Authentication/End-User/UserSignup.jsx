...
export const UserSignup = (props) => {
    ...
    const validateInputs = () => {
        ...
        if (!userId.value || userId.value.length < 1) {
            setUserIdError(true);
    ...
    const handleSubmit = async (event) => {
        ...
        if (nameError || emailError || passwordError || userIdError) {
            return;
        }
        ...
        try {
            const response = await axios.post("http://localhost:8000/user", { name, email, password, userId });
            ...
            <UserRegistrationContainer direction="column" justifyContent="space-between">
                <Card variant="inlined">
                    ...
                    <Box component="form" onSubmit={handleSubmit} sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                        <FormControl>
                            <FormLabel htmlFor="userId">Unique User ID</FormLabel>
                            <TextField sx={{ width: "390px" }} autoComplete="userId" name="userId" required fullWidth id="userId" placeholder="U12345" error={userIdError} helperText={userIdErrorMessage} color={userIdError ? "error" : "primary"} />
                        </FormControl>
